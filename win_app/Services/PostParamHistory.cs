...
namespace SOM.Services
{
    public class PostParamHistory
    {
        public static async Task<HttpResponseMessage> PostParamHistoryAsync(string action, string param_id, string old_value = null, string new_value = null)
        {
            ...
            string jsonData = $"{{ \"action\": \"{action}\", \"param\": \"{param_id}\", \"old_value\": {old_value}, \"new_value\": {new_value}}}";
            if (new_value == null && old_value != null) jsonData = $"{{ \"action\": \"{action}\", \"param\": \"{param_id}\", \"old_value\": {old_value}}}";
            if (new_value != null && old_value == null) jsonData = $"{{ \"action\": \"{action}\", \"param\": \"{param_id}\", \"new_value\": {new_value}}}";
